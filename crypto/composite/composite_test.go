...
func TestGenPrivKey(t *testing.T) {
	...
	blsKey := sk.BlsKey
	signKey := sk.SignKey
	compositeKey := composite.NewPrivKeyComposite(blsKey, signKey)
...
func TestPrivKeyComposite_Bytes(t *testing.T) {
	...
	sk := composite.NewPrivKeyComposite(blsKey, signKey)
	compositeKey := composite.PrivKeyFromBytes(sk.Bytes())
	assert.Equal(t, blsKey, compositeKey.BlsKey)
	assert.Equal(t, signKey, compositeKey.SignKey)
	assert.Equal(t, sk, compositeKey)
}
...
func TestPrivKeyComposite_Identity(t *testing.T) {
	...
	signKey := ed25519.GenPrivKey()
	sk := composite.NewPrivKeyComposite(blsKey, signKey)
	if !bytes.Equal(sk.Identity().Bytes(), signKey.Bytes()) {
...
func TestPrivKeyComposite_PubKey(t *testing.T) {
	...
	assert.Equal(t, blsKey.PubKey(), compositeKey.BlsKey)
	assert.Equal(t, signKey.PubKey(), compositeKey.SignKey)
...
func TestPubKeyComposite_Address(t *testing.T) {
	...
	if len(pk.Address().Bytes()) != len(signKey.PubKey().Address().Bytes()) {
...
func TestPubKeyComposite_Equals(t *testing.T) {
	...
	if pk1.Equals(blsKey.PubKey()) || pk1.Equals(signKey.PubKey()) {
		t.Errorf("The different kind of keys are evaluated as the same")
	}
}
...
func TestPubKeyComposite_Identity(t *testing.T) {
	...
	sk := composite.NewPrivKeyComposite(blsKey, signKey)
	pk := sk.PubKey().(composite.PubKey)
	assert.Equal(t, signKey.PubKey(), pk.Identity())
}
